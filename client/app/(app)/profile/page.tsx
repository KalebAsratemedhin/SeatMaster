...
export default function ProfilePage() {
  ...
  const handleAvatarChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (avatarFileError(file)) return;
    setAvatarFile(file);
    ...
    <div className="container max-w-2xl mx-auto">
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-muted-foreground text-sm">Update your name, phone, and profile picture.</p>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="flex flex-col items-center gap-4">
                <div className="relative">
                  <Avatar className="h-24 w-24 border-2 border-border">
                    {displayAvatar ? <AvatarImage src={displayAvatar} alt="Profile" className="object-cover" /> : null}
                  ...
                  <label className="absolute bottom-0 right-0 flex h-9 w-9 cursor-pointer items-center justify-center rounded-full bg-primary text-primary-foreground shadow-md hover:bg-primary/90">
                    ...
                    <input type="file" accept="image/jpeg,image/png,image/gif,image/webp" className="sr-only" onChange={handleAvatarChange} />
                  </label>
                </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Input id="first_name" value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder="First name" className="rounded-lg" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="last_name">Last name</Label>
                  <Input id="last_name" value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder="Last name" className="rounded-lg" />
                </div>
              </div>
              <div className="space-y-2">
                <Label htmlFor="phone">Phone number</Label>
                <Input id="phone" type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+1 (555) 000-0000" className="rounded-lg" />
              </div>
